...
var src_default = async (req) => {
  const secret = process.env.LEDGER_KEY || "";
  if (secret && req.headers.get("x-ledger-key") !== secret) {
    return json({ error: "unauthorized" }, 401);
  }
  const url = new URL(req.url);
  const what = url.searchParams.get("what") || "state";
  try {
    const store = getStore({ name: "balcony-ledger", consistency: "strong" });
    if (what === "state") {
      if (req.method === "GET") {
      ...
      if (req.method === "POST") {
        ...
        body.at = body.at || (/* @__PURE__ */ new Date()).toISOString();
        await store.setJSON("state", body);
        return json({ ok: true, at: body.at });
      }
    }
    if (what === "bulk") {
      if (req.method !== "POST") return json({ error: "POST required" }, 405);
      const { keys } = JSON.parse(await req.text());
      if (!Array.isArray(keys)) return json({ error: "keys array required" }, 400);
      const wanted = keys.slice(0, 60);
      const out = {};
      await Promise.all(
        wanted.map(async (k) => {
          const d = await store.get(PHOTO_PREFIX + encodeURIComponent(k));
          if (d) out[k] = d;
        })
      );
    ...
    if (what === "index") {
      ...
      return json({
        keys: blobs.map((b) => decodeURIComponent(b.key.slice(PHOTO_PREFIX.length)))
      });
    }
    if (what === "photo") {
      const key = url.searchParams.get("key");
      if (!key) return json({ error: "key required" }, 400);
      const blobKey = PHOTO_PREFIX + encodeURIComponent(key);
      if (req.method === "GET") {
        const data = await store.get(blobKey);
        return data ? json({ key, data }) : json({ error: "not found" }, 404);
      }
      if (req.method === "POST") {
        const body = JSON.parse(await req.text());
        if (typeof body?.data !== "string") return json({ error: "data required" }, 400);
        if (body.data.length > MAX_PHOTO) return json({ error: "photo too large" }, 413);
        await store.set(blobKey, body.data);
        return json({ ok: true });
      }
